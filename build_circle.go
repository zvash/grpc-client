...
func (a BuildCircleArgs) GetMap() map[string]string {
	args := make(map[string]string)
	args["title"] = a.Title
	args["description"] = a.Description
	args["circle_type"] = a.CircleType
	args["is_private"] = a.IsPrivate
...
func (a BuildCircleArgs) Invoke() {
	...
	stream, err := gatewayClient.BuildCircle(requestContext)
	if err != nil {
	...
	req := &pb.BuildCircleRequest{
		Data: &pb.BuildCircleRequest_Info{
			Info: &pb.CircleInfo{
				Title:       a.Title,
				Description: a.Description,
				CircleType:  a.CircleType,
				IsPrivate:   "true" == strings.ToLower(a.IsPrivate),
				ImageExt:    extension,
			},
		},
	}
	...
	for {
		...
		req := &pb.BuildCircleRequest{
			Data: &pb.BuildCircleRequest_ChunkData{
				ChunkData: buffer[:n],
			},
		}
